...
const schema = yup.object().shape({
  ...
  coverImage: yup.mixed()
    .test('required', 'Cover image is required', value => Boolean(value))
    ...
    try {
      ...
      Object.keys(data).forEach(key => {
        if (key === 'tags') {
          ...
          // Explicitly log image file details
          const file = data[key];
          if (file) {
      ...
      // Use the API utility
      const result = await postFormData('/posts/', formData);
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
              ...
              <div className="flex justify-end pt-6">
                <Button
                  ...
                  className="w-full sm:w-auto flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 shadow-md hover:shadow-lg transform transition-all duration-200 hover:-translate-y-0.5"
                >
                  {isSubmitting ? (
                    <>
